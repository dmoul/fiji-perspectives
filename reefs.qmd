...
sf_use_s2(FALSE)
# following https://github.com/r-spatial/sf/issues/2366
...
```{r}
#| label: get-global-reef-and-osm-data
...
# SE: -19.260603, -178.323144 # But don't want to cross 180°
# NW
...
my_proposed_bbox <- matrix(c(xmin, ymin, xmax, ymax),
...
# fj_coastline_fname <- here("data/processed/fj_coastline.rds")
# 
# if(!file.exists(fj_coastline_fname)) {
#   
#   fj_coastline <- my_bbox_fj |>
#     opq()%>%
#     add_osm_feature(key = "natural", 
#                     value = c("coastline")) %>%
#     osmdata_sf()
#   
#   write_rds(fj_coastline, fj_coastline_fname)
#   
# } else {
#   
#   fj_coastline <- read_rds(fj_coastline_fname)
#   
# }

fj_coastline <- my_bbox_fj %>% # my_proposed_bbox %>%
  opq() %>%
  add_osm_feature(key = "natural", 
                  value = c("coastline")) %>%
  osmdata_sf()

fj_islands <- my_bbox_fj %>% #my_proposed_bbox %>%
  opq() %>%
  add_osm_feature(key = "place",
                  value = c("island")) %>%
  osmdata_sf()
# plot(fj_islands$osm_multipolygons$geometry)
...
my_bbox_fj_yasawas <- c(xmin = xmin, ymin = ymin, xmax = xmax, ymax = ymax)

# fj_coastline_yasawas_fname <- here("data/processed/fj_coastline_yasawas.rds")

# if(!file.exists(fj_coastline_yasawas_fname)) {
#   
#   fj_coastline_yasawas <- my_bbox_fj_yasawas |>
#     opq()%>%
#     add_osm_feature(key = "natural", 
#                     value = c("coastline")) %>%
#     osmdata_sf()
#   
#   write_rds(fj_coastline_yasawas, fj_coastline_yasawas_fname)
#   
# } else {
#   
#   fj_coastline_yasawas <- read_rds(fj_coastline_yasawas_fname)
#   
# }
...
fj_islands_yasawas <- my_bbox_fj_yasawas |> 
  opq()%>%
  add_osm_feature(key = "place",
                  value = c("island")) %>%
  osmdata_sf()
...
dta_for_plot |>
  ggplot() +
  geom_sf(data = fj_islands$osm_multipolygons,
          color = "brown", fill = "brown", linewidth = 0.001, alpha = plot_alpha) +
  geom_sf(data = fj_islands$osm_polygons,
          color = "brown", fill = "brown", size = 0.001, alpha = plot_alpha) +
  geom_sf(data = fj_coastline$osm_polygons,
          color = "brown", fill = NA, linewidth = 0.001, alpha = plot_alpha) +
  geom_sf(fill = "goldenrod", color = "goldenrod", alpha = 0.3) +
  coord_sf(xlim = my_bbox_fj[c(1, 3)], # min & max of x values
           ylim = my_bbox_fj[c(2, 4)]) +
...
#| fig-cap: "Top source of global reef data"
#| message: false
#| warning: false
...
The "Global distribution of warm-water coral reefs" dataset draws most of it's data from the Millennium Coral Reef Mapping Project. The third-most common source was unique to Fiji: *Exposed Reefs* - Fiji Lands Information System, Department of Lands and Survey (Ministry of Lands, Mineral Resources & Environment for Fiji).
...
dta_for_plot <- d_raw |>
  st_crop(my_bbox_fj_yasawas)

dta_for_plot |>
  ggplot() +
  geom_sf(data = fj_coastline_yasawas$osm_points,
          color = "darkgrey",
          size = 0.001) +
  geom_sf(data = fj_coastline_yasawas$osm_polygons,
          color = "darkgrey", 
          fill = "darkgrey",
          size = 0.001) +
  geom_sf(data = fj_islands_yasawas$osm_polygons,
          color = "darkgrey", 
          fill = "darkgrey",
          size = 0.001) +
...
d_nacula_places <- my_bbox_fj_nacula |>
  opq()%>%
  add_osm_features(
    features = list(
      "place" = "village",
...
dta_for_plot  |>
  ...
  geom_sf(data = fj_islands_yasawas$osm_polygons |>
          ...
          fill = "darkgrey",
          size = 0.001) +
  geom_sf(data = d_nacula_places$osm_polygons |>
            st_crop(my_bbox_fj_nacula),
          color = "blue", shape = 15,
          size = 2) +
  geom_sf_text(data = d_nacula_places$osm_points |>
                 st_crop(my_bbox_fj_nacula),
               aes(label = name),
               color = "black", hjust = 0,
               size = 3) +
...
ggplot() +
  ...
  geom_sf(data = fj_islands_yasawas$osm_polygons |>
          ...
          fill = "darkgrey",
          size = 0.001) +
  geom_sf(data = reefs$osm_multipolygons |>
            st_shift_longitude() |>
            st_crop(my_bbox_fj_nacula),
          color = "firebrick", fill = "firebrick", alpha = 0.3) +
  geom_sf(data = reefs$osm_points |>
            st_shift_longitude() |>
            st_crop(my_bbox_fj_nacula),
